...
반드시 유효한 JSON만 출력합니다. 마크다운, 코드블록, 설명 없이 JSON 객체만 반환합니다.`;
  ...
  const user = `# 미배정 태스크 (위에서 아래로 진행 순서)
...
2. 작업 연속성: 한 태스크의 산출물을 다음 태스크가 이어받는 관계로 보이면(title·suggestedRole이 연결됨), 앞 태스크 담당자에게 뒤 태스크도 우선 배정해 작업 맥락이 끊기지 않게. 단 한 사람에게 과도하게 몰리면 이 원칙보다 부하 균형을 우선.
3. 부하 균형: 이미 맡은 태스크(currentTasks)가 적은 팀원에게 우선 배정.
4. 쏠림 방지: 같은 성격의 태스크가 한 사람에게 몰리지 않게. currentTasks를 보고 균형을 맞추세요.
5. 마감 우선: dueDate가 임박한 태스크부터 여유 있는 팀원에게 배정.
6. 전원 미선택 시: 아무도 currentTasks가 없으면, 역할 매칭을 먼저 적용한 뒤 남는 태스크를 인원수로 최대한 고르게 나누고, 특정 1인에게 몰지 마세요.
7. rolePreference가 "any"인 팀원은 역할 매칭이 끝난 뒤 남는 태스크를 받습니다.
8. 모든 태스크는 정확히 한 명에게 배정되어야 합니다.
...
- 같은 내용을 다른 표현으로 반복하지 마세요. 한 사실은 한 번만 말하세요.
- 상황 진단보다 실행 제안에 비중을 두세요. (1)~(3)은 짧게 사실만 전달하고, (4) 다음 주 우선순위에 가장 많은 분량을 쓰세요.
- 다음 주 우선순위는 반드시 구체적인 태스크 이름을 들어 제안하세요. "지연 원인을 파악한다", "일정을 재점검한다" 같은 절차적 표현은 쓰지 마세요. 대신 "OO 태스크를 먼저 끝내고 △△로 넘어간다"처럼 무엇을 어떤 순서로 할지 명시하세요.
- 형용사와 부사를 쓰지 마세요. "심각하게", "상당한", "면밀히", "매우" 같은 수식어 없이 사실만 쓰세요.
